        ...
        <div className="min-h-screen flex flex-col bg-[#f0f2f0] dark:bg-neutral-950 text-slate-900 font-sans">
            ...
            <div className="flex items-center justify-between p-4 md:px-6">
                ...
                <button onClick={() => router.push('/signUp')}
                        className="bg-white dark:bg-transparent text-[#0F5525] dark:text-neutral-300 border-[1.5px] border-[#0F5525] dark:border-neutral-700 rounded-lg px-4 py-1.5 text-[13px] font-medium transition-colors hover:bg-[#3FCD6B] dark:hover:bg-neutral-800 hover:border-[#3FCD6B] dark:hover:border-neutral-600 hover:text-white">
                    ← back
                </button>
            </div>
                    ...
                    {activePanel === '2fa' && (
                        <div>
                            <div className="pt-6 px-6 pb-4 md:pt-7.5 md:px-7 md:pb-4.5">
                                <div className="text-[19px] font-semibold text-[#111] dark:text-white mb-1">Two-step verification</div>
                                <div className="text-[13px] text-[#777] dark:text-neutral-400">Confirm it&#39;s you to keep your account secure</div>
                            </div>

                            <div className="px-6 pb-6 md:px-7 md:pb-7">
                                <div className="text-center mb-4">
                                    <div className="w-11 h-11 rounded-full bg-[#E1F5EE] dark:bg-transparent dark:border dark:border-[#3FCD6B]/30 flex items-center justify-center mx-auto mb-2.5">
                                        <i className="ti ti-device-mobile text-[20px] text-[#0F5525] dark:text-[#3FCD6B]"></i>
                                    </div>
                                    <div className="text-[12px] text-[#777] dark:text-neutral-400 leading-relaxed">
                                        Enter the 6-digit code from your authenticator app or sent to <strong className="text-[#111] dark:text-neutral-200">+44 •••• ••7821</strong>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-6 gap-1.5 my-3">
                                    {otp.map((digit, index) => (
                                        <input key={index} ref={el => { otpRefs.current[index] = el; }} maxLength={1} value={digit}
                                               ...
                                               className={`w-full aspect-square text-center text-[18px] font-medium border rounded-lg outline-none transition-all focus:border-[#3FCD6B] focus:ring-[3px] focus:ring-[#3FCD6B]/10 dark:focus:ring-transparent ${digit ? 'border-[#3FCD6B] bg-[#f0fdf5] dark:bg-[#3FCD6B]/5 dark:text-white' : 'border-[#dde3e0] dark:border-neutral-800 bg-white dark:bg-neutral-950 dark:text-white'}`}/>
                                    ))}
                                </div>

                                <div className="text-[11px] text-[#bbb] dark:text-neutral-500 text-center mb-4">
                                    Didn&#39;t receive a code? <span onClick={resendVerificationCode} className="text-[#0F5525] dark:text-[#3FCD6B] cursor-pointer font-medium hover:underline">Resend</span>
                                </div>

                                <button onClick={handleSignUpSubmit}
                                        className="w-full bg-[#0F5525] dark:bg-[#3FCD6B] text-white dark:text-neutral-950 rounded-lg p-2.5 text-[13px] font-medium flex items-center justify-center gap-1.5 transition-colors hover:bg-[#3FCD6B] dark:hover:opacity-90">
                                    <i className="ti ti-shield-check text-[14px]"></i> Verify &amp; sign in
                                </button>

                                <div onClick={() => router.push('/signUp')}
                                     className="text-[12px] text-[#aaa] dark:text-neutral-500 cursor-pointer flex items-center gap-1 mt-3.5 justify-center transition-colors hover:text-[#3FCD6B] dark:hover:text-[#3FCD6B]">
