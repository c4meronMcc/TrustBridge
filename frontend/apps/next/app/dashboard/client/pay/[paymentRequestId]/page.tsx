...
import DashboardMockCheckoutForm from './DashboardMockCheckoutForm';
...
export default function DashboardCheckoutPage() {
    const params = useParams<{ paymentRequestId: string }>();
    ...
    useEffect(() => {
        const loadCheckoutData = async () => {
            try {
                const response = await fetch(`http://localhost:8080/api/payment-requests/${paymentRequestId}`);
    ...
    const fmtAmount = checkout.milestoneAmount.toLocaleString('en-GB', { minimumFractionDigits: 2 });
        ...
        <div className="min-h-screen bg-[#f4f6f1] dark:bg-[#0e1410] pt-10 px-4 pb-12 font-sans transition-colors duration-200">
            ...
            <div className="max-w-[1024px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 lg:items-start">
                ...
                <div className="lg:col-span-5 order-1 lg:order-2 space-y-4 lg:sticky lg:top-6">
                    {/* Insert your existing summary box component here */}
                </div>
            </div>
        </div>
    );
