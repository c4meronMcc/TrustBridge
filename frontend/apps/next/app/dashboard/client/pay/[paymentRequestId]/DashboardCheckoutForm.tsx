...
interface DashboardCheckoutFormProps {
    amount: string;
    symbol: string;
...
export default function DashboardCheckoutForm({ amount, symbol, paymentRequestId, clientName, clientEmail, clientSecret }: DashboardCheckoutFormProps) {
    ...
    const dashboardSuccessUrl = `http://localhost:3000/dashboard/client/pay/${paymentRequestId}/success`;
    ...
    const handleBankTransfer = async (e: React.FormEvent) => {
        ...
        const { error } = await stripe.confirmPayByBankPayment(clientSecret, {
            ...
            return_url: dashboardSuccessUrl,
        });
    ...
    const handleCardPayment = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            window.location.href = dashboardSuccessUrl;
        }
        setIsLoading(false);
    };
